...
uint8_t Rc522Test(void)										
{
    ...
    if (PcdRequest(REQ_ALL, Temp) == MI_OK)
	{
		...
		{
			cardno=0;	
			if(UID[0]==0x43&&UID[1]==0x28&&UID[2]==0xED&&UID[3]==0x10)//ID卡的UID
			{
				cardno=1;
			}
			else if(UID[0]==0x5A&&UID[1]==0XFD&&UID[2]==0X8A&&UID[3]==0X10)//手机NFC卡号UID
			{
				cardno=2;
			}
			else if(UID[0]==UI2[0]&&UID[1]==UI2[1]&&UID[2]==UI2[2]&&UID[3]==UI2[3])//备用
			{
				cardno=3;
			}
			else if(UID[0]==UI3[0]&&UID[1]==UI3[1]&&UID[2]==UI3[2]&&UID[3]==UI3[3])//备用
			{
				cardno=4;
			}
			else cardno = 0;
