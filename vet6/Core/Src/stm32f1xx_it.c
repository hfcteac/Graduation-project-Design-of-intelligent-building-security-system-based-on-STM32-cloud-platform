...
#define BUFFERSIZE2 255	//可接收的最大数据量
extern uint8_t Rx_len2,bootfirst2;
extern uint8_t ReceiveBuff2[BUFFERSIZE2]; //接收缓冲区
/* USER CODE END EV */
  ...
  /* USER CODE BEGIN USART2_IRQn 1 */
	...
	if(USART2 == huart2.Instance)//判断是否为串口2中断
	{
		if(RESET != __HAL_UART_GET_FLAG(&huart2,UART_FLAG_IDLE))//如果为串口2空闲
		{
			...
			temp2  = __HAL_DMA_GET_COUNTER(&hdma_usart2_rx);//获取DMA当前还有多少未填充
			Rx_len2 =  BUFFERSIZE2 - temp2; //计算串口接收到的数据个数
//			OLED_ShowCHinese(6,0,43);//清除最后一行方便刷新用的
//			OLED_ShowCHinese(6,16,43);//清除最后一行方便刷新用的
//      OLED_ShowCHinese(6,32,43);//清除最后一行方便刷新用的		
//      OLED_ShowCHinese(6,48,43);//清除最后一行方便刷新用的	
...
//			OLED_ShowString(0,6,ReceiveBuff,16);
			//HAL_UART_Transmit_DMA(&huart2,ReceiveBuff,Rx_len);//发送数据检验
			Rx_len2=0;//接收数据长度清零
			HAL_UART_Receive_DMA(&huart2,ReceiveBuff2,BUFFERSIZE2);//开启下一次接收
		}	
	}
